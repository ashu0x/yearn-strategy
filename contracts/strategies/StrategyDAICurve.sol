...
contract StrategyDAICurve {
    ...
    function deposit() public {
        ...
        if(_ycrv>0){
            IERC20(ycrv).approve(yycrv, _ycrv);
    ...
    function withdraw(IERC20 _asset) public returns(uint256 balance) {
        ...
        IERC20(_asset).approve(controller, balance);
        IERC20(_asset).transfer(controller, balance);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        IERC20(want).transfer(_vault, _amount);
    }
    ...
    function withdrawUnderlying(uint256 _amount) internal returns(uint256) {
        IERC20(ycrv).approve(curve, _amount);
    ...
    function balanceOf() public view returns(uint256){
        return balanceOfWant().add(balanceOfYCRVyTUSD());
    }
